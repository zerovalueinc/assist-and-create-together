...
export async function ingestHubspotData({ workspace_id, user_id, access_token }: { workspace_id: string, user_id: string, access_token: string }) {
  ...
  try {
    let after = undefined;
    do {
      const url = `https://api.hubapi.com/crm/v3/objects/deals?limit=100${after ? `&after=${after}` : ''}`;
      const res = await axios.get(url, { headers: { Authorization: `Bearer ${access_token}` } });
  ...
  try {
    let after = undefined;
    do {
      const url = `https://api.hubapi.com/crm/v3/objects/notes?limit=100${after ? `&after=${after}` : ''}`;
      const res = await axios.get(url, { headers: { Authorization: `Bearer ${access_token}` } });
