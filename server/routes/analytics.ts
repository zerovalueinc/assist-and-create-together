...
(router.get('/lead-volume', authenticateToken, (req, res, next) => {
  const userId = req.user.id;
  supabase.rpc('lead_volume_by_week_warehouse', { user_id: userId })
    .then(({ data, error }) => {
      if (error) {
...
(router.get('/success-rate', authenticateToken, (req, res, next) => {
  const userId = req.user.id;
  supabase
    .from('crm_deals')
    .select('data')
