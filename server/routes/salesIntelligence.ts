...
router.get('/analytics', authenticateToken, async (req, res) => {
  try {
    ...
    const analytics = {
      ...
      averageICPFitScore: topReports.reduce((sum: number, report: any) => sum + (report.icpFitScore || 0), 0) / topReports.length,
      averageIBPMaturityScore: topReports.reduce((sum: number, report: any) => sum + (report.ibpMaturityScore || 0), 0) / topReports.length,
      averageSalesTriggerScore: topReports.reduce((sum: number, report: any) => sum + (report.salesTriggerScore || 0), 0) / topReports.length,
      averageTotalScore: topReports.reduce((sum: number, report: any) => sum + (report.totalScore || 0), 0) / topReports.length,
      priorityBreakdown: {
        high: topReports.filter((r: any) => r.priority === 'high').length,
        medium: topReports.filter((r: any) => r.priority === 'medium').length,
        low: topReports.filter((r: any) => r.priority === 'low').length
      },
